...
export function useCardStack<T extends BaseCardData>(
  ...
  const moveStack = (dragXPos: number) => {
    ...
    emit('move', activeCardOffset / (config.value.cardWidth + config.value.paddingHorizontal))
    ...
    stack.value = stack.value.map((card, index) => {
      ...
      return {
        ...
        xPos,
        scale,
        opacity: index === 0 && !dragHandling.isDraggingRight.value ? 1 : (defaults.opacity ?? 1)
      }
    })
  }
